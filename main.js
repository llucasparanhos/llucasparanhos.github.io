...
const CASES={
  wellhub:{
    ...
    medicao:{texto:'Sessões de teste moderadas com 5 usuários reais usando protótipo no Figma. Critério de aprovação: conclusão das tarefas principais sem bloqueio crítico.',tools:['Figma Prototype','Teste de Usabilidade','Apoio de IA (Claude), documentação e estruturação do case']},
    en:{
      ey:'UX/UI · Gamification · Wellness',
  ...
  checkout:{
    ...
    medicao:{texto:'GA4 com funil de eventos comparando 30 dias antes vs 30 dias após o lançamento. Microsoft Clarity para análise de gravações de sessão e identificação visual dos pontos de abandono.',tools:['Google Analytics 4','Microsoft Clarity','Apoio de IA (Claude), documentação e estruturação do case']},
    en:{
  ...
  bomconsorcio:{
    ...
    medicao:{texto:'O diagnóstico partiu do time de suporte, a categoria mais frequente de tickets era dúvida sobre andamento do processo, não erros técnicos. Esse padrão qualitativo validou o problema antes do primeiro wireframe e serviu como baseline para a hipótese de redução. Os resultados são projetados, o acesso aos dados pós-lançamento não estava no escopo do projeto.',tools:['Suporte, categorização qualitativa de tickets','Análise de padrão de chamados','Claude Code, apoio ativo no fluxo de trabalho']},
